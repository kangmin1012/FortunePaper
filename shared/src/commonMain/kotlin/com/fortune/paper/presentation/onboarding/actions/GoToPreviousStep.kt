...
/** 이전 단계로 이동. */
data object GoToPreviousStep : ViewAction<OnboardingState, OnboardingEvent, OnboardingDependencies>() {
    override suspend fun execute(
        dependencies: OnboardingDependencies,
        scope: ActionScope<OnboardingState, OnboardingEvent>
    ) {
        if (scope.currentState.step.isFirst) return
        scope.setState { copy(step = step.previous(), error = null) }
    }
}
